...
func WalkingSpentCalories(steps int, weight, height float64, duration time.Duration) (float64, error) {
	...
	// Формула для ходьбы: ((вес * скорость * время_в_минутах) / 60) * 0.5
	// ГАРАНТИРОВАННО применяем коэффициент 0.5 для ходьбы!
	calories := (weight * speed * durationMinutes) / minInH * 0.5
